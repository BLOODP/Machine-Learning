...
public class KmeansDriver implements OperationalDataOnHadoop {
	...
	public void run(boolean isReduce) throws IOException,
		...
		DistributedCache.addFileToClassPath(new Path("/input/rubic.jar"), conf,fs);
...
//		job.setNumReduceTasks(2);
	...
	public String run(HadoopConfiguration configuration)
		...
		double[] parameters = configuration.getParameters();
		init(datasetPath, subpath, parameters);
		if (!kmeansPrepare.checkInput()) {
			LOG.error("error input format for mapreduced kmeans algorithm");
			throw new ErrorInputFormatException();
		}
...
//			FileSystem fs = HDFSConnection.getFileSystem();
			Configuration conf = new Configuration();
			FileSystem fs = new Path(dataSource).getFileSystem(conf);
...
//			HDFSConnection.close(fs);
	...
	public static void main(String[] args) throws Exception {
		...
		String dataSource = "hdfs://master:9000/kmeans/input/wine.data";
		String subPath = "hdfs://master:9000/kmeans+iris/output";
		if(fileDao.isExist(subPath)){
			fileDao.remove(subPath);
		}
		
		
		double parameters = 3;
		HadoopConfiguration conf = new HadoopConfiguration.HadoopConfigurationBuilder(dataSource,subPath).parameters(new double[]{parameters}).build();
		try {
//			String result = kd.run(dataSource, subPath, null,parameters);
			String result = kd.run(conf);
